...
const createOfferingSubHandlers = (tableName, scTableName, allowedColumns) => {
    ...
    const create = async (req, res) => {
        try {
            ...
            if (error) throw new Error(error.message);
            res.status(201).json({ message: `${tableName} created`, data });
        } catch (error) {
...
const feeHandlers = createOfferingSubHandlers(
    ...
    ['student_category', 'payment_plan_type', 'currency', 'amount', 'finance_whitelist_json']
);
...
const eligibilityHandlers = createOfferingSubHandlers(
    ...
    [
        'academic_level_required', 'min_score_percent_gen', 'min_score_percent_reserved',
