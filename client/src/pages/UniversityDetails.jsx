...
const UniversityDetails = () => {
    ...
    const cardToColumn = {
        ...
        'corporate-hiring': null,         // standalone endpoint
    };
    ...
    const ENTITY_TABLE_CONFIG = {
        ...
        'corporate-hiring': { columns: [{ h: 'COMPANY NAME', k: 'company_name' }, { h: 'INDUSTRY DOMAIN', k: 'industry_domain_tag' }, { h: 'LOGO URL', k: 'logo_asset_url' }] },
        ...
        eligibility: { columns: [{ h: 'ACADEMIC LEVEL REQUIRED', k: 'academic_level_required' }, { h: 'MIN SCORE (GENERAL)', k: 'min_score_percent_gen' }, { h: 'MIN SCORE (RESERVED)', k: 'min_score_percent_reserved' }] },
        'fee-structure': { columns: [{ h: 'STUDENT CATEGORY', k: 'student_category' }, { h: 'PAYMENT PLAN TYPE', k: 'payment_plan_type' }, { h: 'CURRENCY', k: 'currency' }, { h: 'AMOUNT', k: 'amount' }] },
    };
    ...
    const valueFieldKeyMap = {
        ...
        'corporate-hiring': {
            'Company Name': 'company_name',
            'Industry Domain': 'industry_domain_tag',
            'Logo URL': 'logo_asset_url',
            'Priority Tier': 'display_priority_tier',
            'Mapped Offering IDs': 'mapped_offering_ids_json',
            'Hiring Intent Category': 'hiring_intent_category',
            'Last Hiring Year': 'last_hiring_year',
            'Total Alumni Hired': 'total_alumni_hired',
            'Logo Usage Rights (Yes/No)': 'logo_usage_rights_status',
            'Career Portal URL': 'career_portal_url',
            'Active (Yes/No)': 'is_active',
        },
    };
    ...
    const fetchUniversityDetails = async (uniId) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setUniversity({
                    ...data,
                    name: data.short_name || data.full_legal_name || 'University',
                });
            } else {
    ...
    const handleCardClick = (cardId) => {
        ...
        if (ENTITY_TABLE_CONFIG[cardId] && cardId !== 'curriculum') {
            ...
            if (cardId === 'corporate-hiring') {
                fetchCorporateHiringData();
            } else if (university?.id && cardToApiPath[cardId]) {
    ...
    const fetchCorporateHiringData = async () => {
        setIsLoading(true);
        try {
            const res = await fetch(`${BACKEND_URL}/api/universities/corporate-hiring-partners`);
            if (res.ok) {
                const result = await res.json();
                setEntityTableData(result.data || []);
            }
        } catch (err) { console.error('Error fetching corporate hiring:', err); } finally { setIsLoading(false); }
    };
    ...
    const renderEntityTable = (cardId, tableData) => {
        ...
        const entityLabels = {
            ...
            'placement-outcome': 'Placement Stat', 'corporate-hiring': 'Corporate Hiring Partner',
    ...
    const handleSaveCard = async () => {
        ...
        // ── Corporate Hiring (standalone value endpoint) ──
        else if (selectedCard === 'corporate-hiring') {
            if (editingEntityId) {
                url = `${BACKEND_URL}/api/universities/corporate-hiring-partners/${editingEntityId}`;
                method = 'PUT';
            } else {
                url = `${BACKEND_URL}/api/universities/corporate-hiring-partners`;
                method = 'POST';
            }
            onSuccess = async () => {
                await fetchCorporateHiringData();
                setCardViewMode('table');
                setEditingEntityId(null);
            };
        }
        ...
        // ── Academics: offering-scoped (fee, eligibility, curriculum, addon) ──
        else if (['fee-structure', 'eligibility', 'curriculum', 'addon'].includes(selectedCard)) {
            if (!selectedProgram) { showToast('Please select a program first.', 'error'); return; }
            const subPathMap = { 'fee-structure': 'fees', 'eligibility': 'eligibility', 'curriculum': 'curriculum', 'addon': 'addons' };
    ...
    const renderCardFields = (cardId) => {
        const fieldConfigs = {
            ...
            'corporate-hiring': [
                { label: 'Company Name', placeholder: 'Enter company name...' },
                { label: 'Industry Domain', placeholder: 'Enter domain...' },
                { label: 'Logo URL', placeholder: 'Enter logo URL...' },
                { label: 'Priority Tier', placeholder: 'Enter tier...' },
                { label: 'Mapped Offering IDs', placeholder: 'Enter IDs JSON...' },
                { label: 'Hiring Intent Category', placeholder: 'Enter category...' },
                { label: 'Last Hiring Year', placeholder: 'Enter year...' },
                { label: 'Total Alumni Hired', placeholder: 'Enter total...' },
                { label: 'Logo Usage Rights (Yes/No)', placeholder: 'Select...', type: 'dropdown', options: ['Yes', 'No'] },
                { label: 'Career Portal URL', placeholder: 'Enter portal URL...' },
                { label: 'Active (Yes/No)', placeholder: 'Select...', type: 'dropdown', options: ['Yes', 'No'] },
            ],
        ...
        const getSourceValue = (fieldLabel) => {
            ...
            if (cardToSourceKey[cardId] !== undefined) {
                if (editingEntityId && editingSourceData) {
                    // Editing a specific entity table row â€” use matched source data
    ...
    const academicsCards = [
        ...
        { id: 'program-specialisation', title: 'Program Specialisation', fieldCount: 6 },
        ...
        // degree-master fields are flat on selectedProgramData.data; others need separate fetch
        const dbData = card.id === 'degree-master' ? selectedProgramData?.data : null;
    ...
    const valueCards = [
        ...
        { id: 'corporate-hiring', title: 'Corporate Hiring Partner', fieldCount: 11 },
    ].map(card => {
    ...
    const openOfferingModal = (offering) => {
        ...
        openModal(`Edit Offering — ${specName}`, { type: 'offering-tabs', offering, activeSubTab: null });
    };
    ...
    const renderModalBody = () => {
                    ...
                    {specs.length > 0 ? (
                        <div className="table-wrapper">
                            <div className="table-header">
                                <div className="table-col" style={{ flex: '1 1 60%' }}>SPECIALIZATION NAME</div>
                                <div className="table-col" style={{ flex: '1 1 40%' }}>ACTIONS</div>
                            </div>
                            {specs.map((spec, index) => (
                                <div className={`table-row ${index % 2 === 0 ? 'row-light' : 'row-white'}`} key={spec.id}>
                                    <div className="table-col" style={{ flex: '1 1 60%' }}>
                                        <span className="uni-name">{spec.specialization_name || 'Unnamed'}</span>
                                    </div>
                                    <div className="table-col" style={{ flex: '1 1 40%' }}>
                                        <button className="action-btn" onClick={() => { closeModal(); setSelectedCard('program-specialisation'); setCardFieldData({}); setEditingSourceData(spec._source_data || spec); }}>Edit</button>
                                    </div>
                                </div>
                            ))}
                        </div>
        ...
        if (type === 'offering-tabs') {
                            ...
                            <div
                                ...
                                onClick={async () => {
                                    ...
                                    if (card.id === 'program-offering' && offering && keyMap) {
                                        ...
                                        setCardFieldData(prefilled);
                                    } else if (['fee-structure', 'eligibility', 'addon'].includes(card.id) && offering?.id) {
                                        // Fetch existing data for offering-scoped sub-entities
                                        const subPathMap = { 'fee-structure': 'fees', 'eligibility': 'eligibility', 'addon': 'addons' };
                                        try {
                                ...
                                style={{
                                    ...
                                    gap: '10px', padding: '20px 12px', border: '1.5px solid #E6E6E6', borderRadius: '12px',
                                    cursor: 'pointer', background: '#FBFBFB', transition: 'border-color 0.2s, box-shadow 0.2s',
                                }}
                                onMouseEnter={e => { e.currentTarget.style.borderColor = '#930051'; e.currentTarget.style.boxShadow = '0 2px 8px rgba(147,0,81,0.1)'; }}
            ...
            // Table view for curriculum, eligibility, fee-structure
            if (['curriculum', 'eligibility', 'fee-structure'].includes(activeSubTab) && cardViewMode === 'table') {
                const config = ENTITY_TABLE_CONFIG[activeSubTab];
                const colWidth = Math.floor(75 / config.columns.length) + '%';
                const titleMap = { curriculum: 'Program Offering Curriculum', eligibility: 'Program Offering Eligibility', 'fee-structure': 'Program Offering Fee Structure' };
                const entityLabel = { curriculum: 'Curriculum Entry', eligibility: 'Eligibility Entry', 'fee-structure': 'Fee Entry' };
                const fetchFn = { curriculum: fetchCurriculumForOffering, eligibility: fetchEligibilityForOffering, 'fee-structure': fetchFeesForOffering };
                return (
                    <div>
                        <button
                        ...
                        <h4 style={{ fontFamily: 'Manrope, sans-serif', fontSize: '14px', fontWeight: 700, color: '#333', marginBottom: '12px' }}>{titleMap[activeSubTab]}</h4>
                        {entityTableData.length > 0 ? (
                            <div className="table-wrapper master-table" style={{ overflowX: 'auto' }}>
                                <div className="table-header">
                                    ...
                                    <div className="table-col" style={{ width: '18%', minWidth: '100px' }}>ACTIONS</div>
                                </div>
                                {entityTableData.map((row, index) => (
                                    <div className={`table-row ${index % 2 === 0 ? 'row-light' : 'row-white'}`} key={row.id || index}>
                                        ...
                                        <div className="table-col action-col" style={{ width: '18%', minWidth: '100px' }}>
            ...
            return (
                <div>
                    <button
                        onClick={() => {
                            const tableSubTabs = ['curriculum', 'eligibility', 'fee-structure'];
                    ...
                    >
                        <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M9 11L5 7L9 3" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>
                        {['curriculum', 'eligibility', 'fee-structure'].includes(activeSubTab) && cardViewMode === 'form' ? 'Back to table' : 'Back to cards'}
                    </button>
                    <h4 style={{ fontFamily: 'Manrope, sans-serif', fontSize: '14px', fontWeight: 700, color: '#333', marginBottom: '12px' }}>{activeCard?.title}</h4>
                    {renderCardFields(activeSubTab)}
                    <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '16px' }}>
                        <button className="add-university-btn" onClick={handleSaveCard}>Save</button>
                    </div>
                </div>
            );
        }
    ...
    const renderContent = () => {
        ...
        const universityMasterCards = [
            ...
            { id: 'corporate-hiring', title: 'Corporate Hiring', fieldCount: 11 },
        ].map(card => {
            const column = cardToColumn[card.id];
        ...
        // ── Table view for entity cards ──
        if (selectedCard && ENTITY_TABLE_CONFIG[selectedCard] && cardViewMode === 'table') {
            ...
            const entityLabels = {
                ...
                'placement-outcome': 'Placement Outcomes', 'corporate-hiring': 'Corporate Hiring',
            };
        ...
        // ── Detail form view when a card is selected ──
        if (selectedCard) {
            const cardLists = {
            ...
            {confirmDialog && (
                <div style={{
                    ...
                    <div style={{
                        ...
                        <h4 style={{ fontFamily: 'Manrope, sans-serif', fontSize: '16px', fontWeight: 700, color: '#1A1A1A', marginBottom: '8px' }}>Confirm Action</h4>
                        ...
                        <div style={{ display: 'flex', gap: '12px', justifyContent: 'center' }}>
                            ...
                            <button
                                ...
                                style={{
                                    ...
                                    background: '#D32F2F', fontFamily: 'Manrope, sans-serif', fontSize: '13px',
